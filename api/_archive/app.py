...
async def live_alerts(timeframe: str = "1d", min_confidence: float = 75.0, limit: int = 10):
    ...
    try:
        _min_conf = float(max(50.0, min(99.0, min_confidence)))
...
async def get_candles(symbol: str, interval: str = "5m", limit: int = 200):
    ...
    try:
        ...
        for d, row in df.iterrows():
            ...
            low_val = _safe_float(low_val, float("nan"))
            c = _safe_float(c, float("nan"))
            v = _safe_float(v, 0.0)

            if np.isnan(o) or np.isnan(h) or np.isnan(low_val) or np.isnan(c):
                continue
